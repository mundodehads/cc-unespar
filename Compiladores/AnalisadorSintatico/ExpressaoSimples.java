...
public class ExpressaoSimples extends Noh {

	private ArrayList<Noh> filhos = new ArrayList<Noh>();

	public ExpressaoSimples(Arvore ar) {
		...
		try {
			...
			if(ar.procurarLexema().equals("+")){
				...
				simboloSimples1(ar, "-");
				filhos.add(new Termo(ar));
			}else if(ar.procurarLexema().equals("or")){
				palavrasReservadas1(ar, "or");
				filhos.add(new Termo(ar));
			}
		} catch (Exception e) {
			e.printStackTrace();
