...
/*
se antes x,y era definido como:
x = hip * cos(a)
y = hip * sen(a)
...
void interacao_mause(int button, int state, int x, int y)
{
    ...
    if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN){
        ...
        // ##3
        // lado esquerdo
        ax2 = x2*cos(inc) - ny * sin(inc);
        ay2 = ny*cos(inc) + x2 * sin(inc);

        // lado dirieto
        ax3 = x3 * cos(inc) - ny * sin(inc);
        ay3 = ny * cos(inc) + x3 * sin(inc);
